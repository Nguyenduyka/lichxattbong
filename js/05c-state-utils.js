...
const save=()=>{
  ...
  try{
    ...
    // QuotaExceededError, SecurityError (private mode), v.v.
    // → Báo user vì offline mode sẽ không hoạt động.
    logWarn('[Save] localStorage failed:', e.name, e.message);
    try{ showToast('⚠️ Bộ nhớ thiết bị đầy — lịch offline có thể chưa cập nhật'); }catch(_){}
  }
  ...
  if(fbReady&&fbStorage&&fbDb&&currentUID){
          ...
          return fetch(savedUrl+(savedUrl.indexOf('?')>=0?'&':'?')+'t='+Date.now(),{cache:'no-store'}).then(r=>r.ok?r.json():null).then(verifyData=>{
            if(Array.isArray(verifyData)){
              if(verifyData.length===events.length){
                log('[Save] ✅ Verified: file content has '+verifyData.length+' events (match)');
              } else {
                logErr('[Save] ❌ FILE CONTENT MISMATCH!');
                log('  Local events:',events.length);
                log('  Uploaded file:',verifyData.length);
                showToast('⚠️ Upload không đầy đủ! Server có '+verifyData.length+' events, local có '+events.length);
              }
            } else {
              logWarn('[Save] ⚠️ Could not verify file content');
            }
    ...
    logWarn('[Save] Firebase chưa sẵn sàng, chỉ lưu localStorage. fbReady=',fbReady,'fbStorage=',!!fbStorage,'currentUID=',currentUID);
    if(!currentUID){
