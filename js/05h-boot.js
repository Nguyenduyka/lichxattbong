...
(async()=>{
  ...
  try{
    ...
    console.log('[Boot] initFirebase...');
    await Promise.race([initFirebase(), _T(6000)]).catch(e=>{
      console.warn('[Boot] initFirebase timeout/error:',e.message);
    });
    console.log('[Boot] fbReady='+fbReady+', events='+events.length);
    setLoadProgress(50);

    // Bước 2: Chờ _loadViewerData load xong từ Firebase (tối đa 60s cho file lớn)
    if(fbReady && events.length===0){
      console.log('[Boot] waiting for Firebase data...');
      await Promise.race([
        new Promise(resolve=>{
        ...
        _T(60000)
      ]).catch(()=>{console.warn('[Boot] Firebase data timeout after 60s');});
      console.log('[Boot] events after Firebase wait='+events.length);
    }

    // Bước 3: Chờ weather xong (tối đa 8s, đã chạy song song nên thường đã có)
    setLoadProgress(80);
    console.log('[Boot] waiting for weather...');
    const wxd=await Promise.race([_wxPromise, _T(8000)]).catch(()=>null);
    if(wxd&&!wxData){wxData=wxd;wxFetchedAt=Date.now();}
    console.log('[Boot] weather ready, wxData='+!!wxData+', mock='+wxMock);
    ...
    trackVisit();
    console.log('[Boot] done, events='+events.length);
  }catch(err){
