...
async function loadApiKeyFromFirebase() {
  ...
  } catch(e) { console.warn('loadApiKeyFromFirebase error', e); }
}
  ...
  try {
    ...
    for (var ki = 0; ki < _geminiKeys.length; ki++) {
      ...
      if (fetchRes.status === 429) {
        ...
        console.warn('Key ' + (ki+1) + ' 429 full:', full429);
      ...
      if (fetchRes.status === 401 || fetchRes.status === 403) {
        ...
        console.warn('Key ' + (ki+1) + ' auth error:', msgAuth);
      ...
      if (!fetchRes.ok) {
        ...
        console.error('Gemini key ' + (ki+1) + ' error:', msgErr);
        continue;
      }
    ...
    if (finishReason === 'MAX_TOKENS') {
      ...
      console.warn('Gemini hit MAX_TOKENS, may have truncated output');
    }
    ...
    if (!parsed) {
      console.warn('AI raw response (không parse được):', rawText);
    ...
    console.error('aiExtractFromFile error:', errCatch);
...
function aiNormalizeResult(r) {
  console.log('[AI] Raw response from Gemini:', JSON.stringify(r, null, 2));
  var out = {};
  ...
  if (out.date) {
    ...
    if (isNaN(dTest.getTime())) {
      console.warn('[AI] Ngày không hợp lệ, bỏ qua:', out.date);
      out.date = '';
    }
  }
  ...
  console.log('[AI] Normalized result:', JSON.stringify(out, null, 2));
...
function aiDoFill() {
  ...
  if (r.date) {
    ...
    if (fD) {
      ...
      if (dateFound) {
        fD.value = r.date;
        console.log('[AI] Set ngày trong tuần hiện tại:', r.date);
      } else {
        ...
        try {
          ...
          console.log('[AI] Đã tự chuyển sang tuần chứa ngày', r.date, '(wkOff='+wkOff+')');
        } catch(e) {
          console.warn('[AI] Không chuyển được tuần:', e.message);
        }
      }
    }
  }
