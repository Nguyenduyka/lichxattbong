...
function _renderNotifPanel(){
  ...
  if(_n===0){
    ...
    list.innerHTML=_npMsgLog.map((it,idx)=>{
      ...
      const dateAttr=it.date?`data-date="${it.date}"`:'';
      const evidAttr=it.evId?`data-evid="${it.evId}"`:'';
      return `<div class="np-item" ${dateAttr} ${evidAttr} data-ts="${it.ts}" onclick="_npItemClick(this)">
        ...
        <div class="np-body">
          <div style="font-size:10px;font-weight:700;color:${color};margin-bottom:3px;letter-spacing:.3px">${meta.label}</div>
          <div class="np-ev-title" style="-webkit-line-clamp:2;display:-webkit-box;-webkit-box-orient:vertical;overflow:hidden">${short.replace(/^[^:]+:\s*/,'')}</div>
...
async function sendFCMPush(count, msg, evId, evDate) {
  console.log('[Push] sendFCMPush called:', {count, msg: msg?.slice(0,50), evId});
  try {
    ...
    if (fbDb) {
      ...
      fbDb.ref('push_trigger/latest').set(payload)
        .then(()=>console.log('[Push] ✅ push_trigger written to Firebase'))
        .catch(e=>console.warn('[Push] ❌ push_trigger failed:',e.message));
    } else {
      console.warn('[Push] fbDb not ready');
    }
    ...
    if (GAS_PUSH_URL) {
      ...
      console.log('[Push] calling GAS:', gasUrl.slice(0,100)+'...');
      fetch(gasUrl, {method:'GET', mode:'no-cors'})
        .then(()=>console.log('[Push] ✅ GAS request sent (no-cors, response opaque)'))
        .catch(e=>console.warn('[Push] ❌ GAS request failed:',e.message));
    } else {
      console.warn('[Push] GAS_PUSH_URL not set');
    }
  } catch(e) {
    console.error('[Push] sendFCMPush error:', e);
  }
}
